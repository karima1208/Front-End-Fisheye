...
fetch('../../data/photographers.json')
    ...
    function updateTotalLikes() {
        const allLikesElements = document.querySelectorAll('.likes-container p');
        let totalLikes = 0;
        
        allLikesElements.forEach(likeElement => {
            const likesText = likeElement.textContent.match(/\d+/); // Extraire le nombre de likes
            if (likesText) {
                totalLikes += parseInt(likesText[0]);
            }
        });
